...
const (
	...
	// before falling back to port-forwarding.
...
// with port-forward as the fallback.
func (h *HubbleSource) Connect(ctx context.Context, contextName string) (*portforward.ConnectionInfo, error) {
	...
	// Manual address: dialed as-is, no fallback (--prometheus-url semantics)
	if manual := hubbleAddress(); manual != "" {
		...
		h.directAddr = manual
		// Drop any leftover traffic port-forward so it can't be misreported
		// as the live connection later
	...
	// Direct pass: dial the relay's Service address (reachable in-cluster
	// unless policy blocks it, and on networks that route Service DNS)
...
// direct (non-port-forward) connection, nil otherwise.
...
// suggests, and installs the connection on success (caller must hold the lock).
